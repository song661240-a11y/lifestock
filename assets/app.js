...
const APP_VERSION = "股票資產 PWA v4.0";
...
const PERSONAL_KEYS = new Set(["sharesTqqq","sharesQqq","sharesSpy","sharesSpyi","sharesQqqi","cashUsd","otherUsd","usdtwd","currency","ftUsd","subUsd","twStockTwd","otherTotalTwd","privacyMode","coverTheme","notes","marketState","hotRank","dcaActive","dcaCompleted","dcaPoolUsd","dcaLastDate","dcaNextDueDate","riskOffCycleId","riskOnCycleId","strategyPhase","parametersLocked","hotAsset"]);
const computePortfolioSummary = (data, strategyUsdValue) => {
    ...
    const ftUsd=getNum(data?.ftUsd), subUsd=getNum(data?.subUsd);
    ...
    const cards=[
        ...
        {key:'sub',label:'複委託',amountText:`US$ ${money(subUsd,2)}`,note:'只計入總資產',ratio:totalTwd>0?subUsd*rate/totalTwd:0},
    ...
    return {rate,strategyUsd,ftUsd,subUsd,twStockTwd,otherTotalTwd,externalTwd,totalTwd,totalDisplay:`NT$ ${money(totalTwd,0)}`,strategyDisplay:`US$ ${money(strategyUsd,2)}`,cards};
};
const withPortfolioSnapshot = (raw, reason='save') => {
    ...
    const date=todayStr();
    const snapshot={date,createdAt:new Date().toISOString(),strategyUsd,totalTwd:summary.totalTwd,rate:summary.rate,ftUsd:summary.ftUsd,subUsd:summary.subUsd,twStockTwd:summary.twStockTwd,otherTotalTwd:summary.otherTotalTwd,reason};
...
const App = () => {
    ...
    const portfolio = useMemo(() => computePortfolioSummary(previewData, metrics.totalUsd), [previewData.ftUsd, previewData.subUsd, previewData.twStockTwd, previewData.otherTotalTwd, previewData.usdtwd, metrics.totalUsd]);
    ...
    const handleHomeSliderScroll = useCallback((e) => {
        ...
        homeScrollTimerRef.current = setTimeout(() => {
            const step = el.clientWidth + 16;
            const idx = Math.max(0, Math.min(3, Math.round(el.scrollLeft / Math.max(1, step))));
    ...
    const Home = () => {
        ...
        return React.createElement("main", { className: "max-w-5xl mx-auto px-4 pt-1 content-bottom-space", onTouchStart:handleHomeTouchStart, onTouchMove:handleHomeTouchMove, onTouchEnd:handleHomeTouchEnd, onTouchCancel:()=>{pullStartRef.current.active=false;setPullDistance(0);} },
            ...
            React.createElement("div", { className:"mb-4 flex items-center justify-between gap-3" },
                ...
                React.createElement("div", { className:"flex gap-1.5" }, [0,1,2].map(idx => React.createElement("button", { key:idx, onClick:()=>scrollHomeTo(idx), className:`h-2.5 rounded-full transition-all ${homeSlide===idx?'w-8 bg-slate-900':'w-2.5 bg-slate-300'}` })))),
    ...
    const Inputs = () => React.createElement("main", { className: "max-w-5xl mx-auto px-4 pt-5 content-bottom-space" },
        ...
        Collapsible({ id: "assetAccounts", title: "全部資產帳戶", desc: "IB 策略帳戶以目前持股自動計算；FT、複委託、台股與其他資產手動輸入。", children:
            React.createElement("div", null,
                React.createElement("div", { className:"grid grid-cols-1 sm:grid-cols-2 gap-3" },
                    ...
                    renderDraftNumInput("ftUsd", "Firstrade 淨值", "USD", "只計入全部總資產，不參與策略。"),
                    renderDraftNumInput("subUsd", "複委託淨值", "USD", "只計入全部總資產，不參與策略。"),
    ...
    const ProfitChart = ({ logs, range, mode, onRange, onMode }) => {
        ...
        const ibRaw=[];
        (Array.isArray(data.portfolioHistory)?data.portfolioHistory:[]).forEach(x=>{if(getNum(x.strategyUsd)>0)ibRaw.push({date:String(x.date||"").slice(0,10),value:getNum(x.strategyUsd),source:'daily'});});
        ...
        const totalRaw=(Array.isArray(data.portfolioHistory)?data.portfolioHistory:[]).map(x=>({date:String(x.date||"").slice(0,10),value:getNum(x.totalTwd),source:'daily'})).filter(x=>x.date&&x.value>=0);
        if(getNum(portfolio.totalTwd)>0) totalRaw.push({date:portfolioCurrentDate,value:getNum(portfolio.totalTwd),source:'current'});
        const raw=mode==="TOTAL"?totalRaw:ibRaw;
        ...
        const currency=mode==="TOTAL"?"TWD":"USD";
        const symbol=currency==="TWD"?"NT$":"$";
        const title=mode==="TOTAL"?"全部總資產":"IB 策略資產";
        ...
        const netFlow=mode==="IB"&&canChart?(Array.isArray(logs)?logs:[]).filter(r=>r.recordType==="cashflow").reduce((sum,r)=>{
            const d=String(r.cashflowDate||r.executionDate||"").slice(0,10);
            if(!d||d<startDate||d>endDate)return sum;
            return sum+(r.cashflowType==="withdrawal"?-1:1)*Math.abs(getNum(r.cashflowAmountUsd));
        },0):0;
        const change=canChart?(mode==="IB"?last-first-netFlow:last-first):0;
        ...
        const selectedFlow=selected&&mode==='IB'?(Array.isArray(logs)?logs:[]).filter(r=>r.recordType==='cashflow'&&String(r.cashflowDate||r.executionDate||'').slice(0,10)===selected.date).reduce((sum,r)=>sum+(r.cashflowType==='withdrawal'?-1:1)*Math.abs(getNum(r.cashflowAmountUsd)),0):0;
        const selectedDelta=selected&&previous?selected.value-previous.value-selectedFlow:0;
        const selectedNetFlow=selected&&mode==='IB'?(Array.isArray(logs)?logs:[]).filter(r=>r.recordType==='cashflow').reduce((sum,r)=>{const d=String(r.cashflowDate||r.executionDate||'').slice(0,10);if(!d||d<startDate||d>selected.date)return sum;return sum+(r.cashflowType==='withdrawal'?-1:1)*Math.abs(getNum(r.cashflowAmountUsd));},0):0;
        ...
        const modeButtons=[["IB","IB"],["TOTAL","全部"]].map(([id,label])=>React.createElement("button",{key:id,onClick:()=>onMode(id),className:`px-3 py-2 rounded-full text-xs font-black ${mode===id?"bg-white text-slate-950":"text-white/65"}`},label));
        const header=React.createElement("div",{className:"flex items-center justify-between gap-3"},
            ...
            React.createElement("div",{className:"flex rounded-full bg-white/10 p-1"},modeButtons));
        const summaryLeft=React.createElement("div",null,
            ...
            React.createElement("div",{className:"text-xs font-bold text-white/55 mt-1"},canChart?`${signedPctText(changePct*100,2)}｜${startDate} ～ ${endDate}`:(mode==="TOTAL"?"全部總資產會從這個版本開始累積快照":"至少需要 2 筆 IB 策略紀錄")));
        ...
        const chartBody=canChart?React.createElement("div",null,
            ...
            selected&&React.createElement("div",{className:"grid grid-cols-3 gap-2 px-2 pb-2"},
                ...
                React.createElement("div",{className:"col-span-3 text-center text-[10px] font-bold text-white/45"},`區間起點以來 ${signedPctText(selectedPct*100,2)}；手指在圖上左右滑動查看`)))
            :React.createElement("div",{className:"h-52 flex items-center justify-center text-center px-6 text-sm font-bold text-white/45 leading-relaxed"},mode==="TOTAL"?"儲存一次正式資料後會建立第一個全部總資產快照；之後每次更新股價、執行或同步都會更新。":"目前區間內的策略資產紀錄不足。");
        ...
        const note=React.createElement("div",{className:"mt-3 text-[10px] font-bold text-white/40 leading-relaxed"},mode==="IB"?"IB 顯示扣除已記錄入金／出金後的資產變化；每天更新股價會保存一筆快照。":"全部總資產包含 IB、FT、複委託、台股與其他資產；每天同日期只保留最新快照。");
...
目前正式策略狀態與雲端歷史紀錄會被替換成備份內的 ${records.length} 筆紀錄。此操作不會刪除本機其他備份卡片。`))return;
    ...
    const Logs = () => React.createElement("main", { className: "max-w-5xl mx-auto px-4 pt-5 content-bottom-space" },
    ...
    const AccountsSettingsPage = () => React.createElement(SettingsPage,{eyebrow:"其他券商",title:"全部資產帳戶",desc:"FT、複委託與台股只計入股票總資產，不參與 TQQQ 策略。"},
        React.createElement(Card,{className:"p-5"},
            React.createElement("div",{className:"rounded-[24px] bg-slate-950 text-white p-4 mb-4"},React.createElement("div",{className:"text-[10px] font-black text-white/55"},"目前全部總資產"),React.createElement("div",{className:"mt-2 text-2xl font-black privacy-value"},portfolio.totalDisplay),React.createElement("div",{className:"mt-1 text-xs font-bold text-white/60 privacy-value"},`IB ${portfolio.strategyDisplay}`)),
            React.createElement("div",{className:"grid grid-cols-1 sm:grid-cols-2 gap-3"},
                renderDraftNumInput("ftUsd","Firstrade 淨值","USD"),
                renderDraftNumInput("subUsd","複委託淨值","USD"),
                renderDraftNumInput("twStockTwd","台股淨值","TWD"),
                renderDraftNumInput("otherTotalTwd","其他股票相關資產","TWD")),
            React.createElement("button",{onClick:manualSave,className:"w-full mt-4 py-4 rounded-[22px] bg-slate-950 text-white font-black"},"儲存並建立今日快照")));
    ...
    const Settings = () => {
        ...
        return React.createElement("main",{className:`max-w-5xl mx-auto px-4 pt-5 content-bottom-space ${settingsMotion==="back"?"page-slide-from-left":"page-fade-in"}`},
    ...
    const BottomNav = () => React.createElement(React.Fragment, null,
        page==='home' && homeSlide===2 && React.createElement("button", { onClick: confirmExecution, disabled:previewActive || !metrics.canExecute || executionPreparing || loadingPrice, className: `fixed bottom-[6.35rem] left-1/2 -translate-x-1/2 z-50 min-w-[190px] px-7 py-3.5 rounded-full text-white text-sm font-black shadow-2xl border border-white/80 active:scale-95 ${!previewActive && metrics.canExecute && !executionPreparing && !loadingPrice?"bg-gradient-to-r from-blue-600 to-indigo-600":"bg-slate-400 opacity-80"}` }, previewActive?"測試預覽中":executionPreparing?"更新股價中…":loadingPrice?"股價更新中":metrics.canExecute?"更新後執行":"資料待修正"),
